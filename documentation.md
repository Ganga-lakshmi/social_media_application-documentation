...
- Users are first need to login to the application then the each user database is created.
...
- Anyone can post the tech feeds, that is users can upload their posts by using the new post (+) icon in their specific post details section using database.

- Users can raise any doubts or suggestions with the comment facility. And these comments are stored in individual user database.
...
## Technologies Used
  ### Nodejs:
  ```{ write about node js why you are using it}```
...
```
​
```{Define more schemas of you own here}```
​
...
```
​
```{Define more routes here of you own here}```
​2) ### POST /user/login
Request body - 
```
{
